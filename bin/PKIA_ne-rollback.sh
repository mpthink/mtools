...
echo "PKIA rollback is starting";
...
SUCCESSFILE=$LOGBASE/PKIA_ne-rollback.SUCCESS 
FAILEDFILE=$LOGBASE/PKIA_ne-rollback.FAILED

if [ -e $LOGBASE/PKIA_ne-rollback.* ]
then
	rm $LOGBASE/PKIA_ne-rollback.*
fi
...
{ 
   ...
   echo "PKIA rollback is finished"
   exit $1
}

if [ -e $FINALFILE ]
then
    sed -i "s/old_IP/$LB_WAS_IP/g" $FINALFILE
    sed -i "s/new_IP/$OSS5_OSSCORE2_IP/g" $FINALFILE
