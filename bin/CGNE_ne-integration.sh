...
echo "CGNE integration is starting";
...
SUCCESSFILE=$LOGBASE/CGNE_ne-integration.SUCCESS 
FAILEDFILE=$LOGBASE/CGNE_ne-integration.FAILED

if [ -e $LOGBASE/CGNE_ne-integration.* ]
then
	rm $LOGBASE/CGNE_ne-integration.*
fi
...
{ 
   ...
   echo "CGNE integration is finished"
   exit $1
}
